  ...
  return (
    <div>
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <h1 className="text-[32px] font-semibold tracking-tight-custom">Saved Ideas</h1>
        <p className="text-muted-foreground mt-1 text-sm">Bookmarked validations for quick access</p>
      </motion.div>

      {savedIdeas.length === 0 ? (
        <div className="surface-card p-12 text-center">
          <Bookmark className="w-10 h-10 text-muted-foreground/30 mx-auto mb-3" />
          <p className="text-[16px] text-foreground/70 mb-1">No saved ideas yet</p>
          <p className="text-[13px] text-muted-foreground">Bookmark a validation to see it here</p>
        </div>
      ) : (
        <div className="space-y-3">
          {savedIdeas.map((idea, i) => (
            <motion.div key={idea.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              className="surface-card p-5 flex items-center justify-between group">
              <div className="flex items-center gap-3">
                <Bookmark className="w-4 h-4 text-primary flex-shrink-0" />
                <div>
                  <h3 className="text-[14px] font-medium">{idea.title}</h3>
                  <div className="flex items-center gap-3 mt-1 text-[11px] text-muted-foreground font-mono">
                    <span className={`font-semibold ${verdictColor[idea.verdict]}`}>{idea.verdict}</span>
                    <span className="text-muted-foreground/30">·</span>
                    <span>{idea.confidence}%</span>
                    <span className="text-muted-foreground/30">·</span>
                    <span>{idea.savedAt}</span>
                  </div>
                  {idea.notes && <p className="text-[11px] text-muted-foreground mt-1 italic font-mono">"{idea.notes}"</p>}
                </div>
              </div>
